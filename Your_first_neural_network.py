...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        delta_weights_i_h = np.zeros(self.weights_input_to_hidden.shape) # 3x2
        delta_weights_h_o = np.zeros(self.weights_hidden_to_output.shape) # 2x1
        for X, y in zip(features, targets):
            #### Implement the forward pass here ####
            ### Forward pass ###
            # print("X shape: ", X.shape) # 1x3
            ...
            #hidden_error = np.dot(error*final_outputs*(1-final_outputs), delta_weights_h_o.T)
            hidden_error = error*self.weights_hidden_to_output.T # (1x2)
            
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            output_error_term = error # (1)
            hidden_error_term = hidden_error*(hidden_outputs*(1-hidden_outputs)) # (1x2)

            # Weight step (input to hidden)
            delta_weights_i_h += X[:,None].dot(hidden_error_term) # 3x2 where X[:,None] (3x1)
    ...
    def run(self, features):
        ...
        #print("[run]hidden_inputs ",hidden_inputs)
        hidden_outputs = self.activation_function(hidden_inputs) # signals from hidden layer
        #print("[run]hidden_outputs ", hidden_outputs)
...
# 结果并不是很理想，在多次调整了超参数以后，还是没法很好的预测Dec 21以后的情况。我想这可能是神经网络的深度不够，或者是神经网络的局限性。
